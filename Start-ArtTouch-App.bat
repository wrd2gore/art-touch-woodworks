...
if exist "%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe" (
    ...
    start "" "%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe" --app="%~dp0admin.html" --window-size=1360,880
    exit
)

if exist "%ProgramFiles%\Google\Chrome\Application\chrome.exe" (
    echo Launching Standalone Control Center App Window...
    start "" "%ProgramFiles%\Google\Chrome\Application\chrome.exe" --app="%~dp0admin.html" --window-size=1360,880
    exit
)
